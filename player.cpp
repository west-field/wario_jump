...
void Player::draw()
{
	...
	{
		//死んだとき右側を表示
		DrawRectGraphF(m_pos.x, m_pos.y, m_graphSize.x / 2, 0, m_graphSize.x / 2, m_graphSize.y, m_handle, true, false);
	}
	else
	{
		//生きているときは左側を表示
		DrawRectGraphF(m_pos.x, m_pos.y, 0, 0, m_graphSize.x / 2, m_graphSize.y, m_handle, true, false);
	}
}
